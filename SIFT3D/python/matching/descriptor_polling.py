...
def train_descriptor_prediction_model(X, y):
    ...
    logging.info("Training the descriptor polling model")
    logging.info("Using " + str(type(model)) + " as a descriptor-level model.")
...
def polling_experiment(dataset_id):
    """
    ...
    model = train_descriptor_prediction_model(X_train, y_train)
...
def polling_experiment_multiple(dataset_id, times):
    """
    ...
    for i in range(0, times):
        yeah, noes = polling_experiment(dataset_id)
        total_yeahs += yeah
        total_noes += noes
...
if __name__ == "__main__":
    polling_experiment_multiple('weissman', 1)