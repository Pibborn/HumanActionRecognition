...
if maxlevel,
    ...
    for level = 1:maxlevel,
        ...
        [vertices, faces] = mesh_refine_tri4(vertices, faces);
        % FV.vertices = sphere_project(FV.vertices,r);
        vertices = sphere_project(vertices, r, [0,0,0]);
...
for i=1:length(faces)
    centers(i,:) = mean(vertices(faces(i,:),:));
    % Unit Normalization
    centers(i,:) = centers(i,:) ./ sqrt(dot(centers(i,:),centers(i,:)));
end
...
FV = struct('vertices',vertices, 'faces',faces,'centers',centers);
return