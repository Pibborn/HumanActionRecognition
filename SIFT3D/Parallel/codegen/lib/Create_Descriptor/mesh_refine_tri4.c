...
void mesh_find_vertex(const emxArray_real_T *Vertices, const double vertex_data[],
                      const int vertex_size[2], emxArray_real_T *vertices,
                      double N_data[], int N_size[1])
{
  emxArray_real_T *Va;
  int Vertices_idx_0;
  int i3;
  ...
  emxEnsureCapacity((emxArray__common *)Vexist, i3, (int)sizeof(double));
  jcol = ii->size[0];
  for (i3 = 0; i3 < jcol; i3++) {
  ...
  if (Vexist->size[0] != 0) {
    ...
    for (i3 = 0; i3 < 3; i3++) {
      jcol = Vertices->size[0];
      for (nx = 0; nx < jcol; nx++) {
        vertices->data[nx + vertices->size[0] * i3] = Vertices->data[nx +
          Vertices->size[0] * i3];
      }
    }

    for (i3 = 0; i3 < 3; i3++) {
      vertices->data[Vertices->size[0] + vertices->size[0] * i3] =
        vertex_data[vertex_size[0] * i3];
    }
