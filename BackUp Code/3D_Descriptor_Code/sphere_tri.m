...
function [FV] = sphere_tri(varargin)
...
coder.varsize('FV.vertices');
coder.varsize('FV.faces');
coder.varsize('FV.centers');

shape = lower(shape);

% switch shape
% case 'tetra'
if strcmp(shape,'tetra')
    % tetra = struct;
    tetra = struct('v',zeros(4,3),'f',zeros(4,3));
    ...
    FV.vertices = tetra.v;
    FV.faces    = tetra.f;
    FV.centers  = zeros(size(tetra.f,1),3);
% case 'oct'
elseif strcmp(shape,'oct')
    % oct = struct;
    oct = struct('v',zeros(6, 3),'f',zeros(8,3));
    ...
    FV.vertices = oct.v;
    FV.faces    = oct.f;
    FV.centers  = zeros(size(oct.f,1),3);
% case 'ico'
else
    ico = struct('v',zeros(12,3),'f',zeros(20,3));
    % ico = struct('v',[],'f',[]);
    ...
    FV.vertices = ico.v;
    FV.faces    = ico.f;
    FV.centers  = zeros(size(ico.f,1),3);
end
% -----------------
...
if maxlevel
    ...
    for level = 1:maxlevel
        ...
        FV.vertices = sphere_project(FV.vertices,r,[0,0,0]);
